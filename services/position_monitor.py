...
async def run_periodic_monitoring(interval_hours: int = 24, delay_first_run: bool = False):
    """
    Запускает мониторинг периодически
    
    Args:
        interval_hours: Интервал между проверками в часах (по умолчанию 24 часа)
        delay_first_run: Если True, первая проверка будет через 1 час вместо немедленно
    """
    logger.info(f"🚀 Запуск периодического мониторинга должностей (каждые {interval_hours} ч)")
    
    # Если нужна задержка перед первым запуском (чтобы не тормозить старт бота)
    if delay_first_run:
        logger.info("⏳ Первая проверка будет выполнена через 1 час")
        await asyncio.sleep(3600)  # 1 час задержки
