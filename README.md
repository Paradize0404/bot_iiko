...
- `python scripts/transactions_console_report.py --from-date 2025-11-01 --to-date 2025-11-27` — выгружает из OLAP TRANSACTIONS сумму прихода (`Sum.Incoming`) с группировкой по типу транзакции, типу номенклатуры, счёту и группе 3-го уровня. По умолчанию фильтрует только счета **«ТМЦ Пиццерия»** и **«Хоз. товары Пиццерия»**, а также тип номенклатуры «Товар (GOODS)». Скрипт выводит таблицу и агрегаты в консоль, что удобно для ручной проверки без запуска бота.
