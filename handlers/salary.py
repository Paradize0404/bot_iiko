...
async def salary_menu(message: Message, state: FSMContext):
    ...
    calendar = build_calendar(today.year, today.month, calendar_id="salary", mode="range")
...
@router.callback_query(F.data.startswith("CAL:salary"))
async def handle_salary_calendar(callback: CallbackQuery, state: FSMContext):
    ...
    if data["action"] in ["PREV", "NEXT"]:
        ...
        if new_month == 0:
            ...
            new_month = 1
            new_year += 1
        calendar = build_calendar(new_year, new_month, calendar_id="salary", mode="range")
